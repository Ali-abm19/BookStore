...
export default function Products({ setCartBooks, cartBooks }) {
  ...
  const url = `https://sda-3-online-backend-teamwork-7fzj.onrender.com/api/v1/Books?limit=${limit > 0 ? Math.round(limit) : 4}&offset=${(page - 1) * limit}&SearchByTitle=${title}&MaxPrice=${max}&MinPrice=${min}&SearchByAuthor=${author}`;
